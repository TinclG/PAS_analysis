...
```{r, eval=FALSE}
#package PcaExplorer topGOtable
#clusterexplorer
#ReactomePA
library(topGO)
library(ALL)
...
genes<-genes%>%
   ...
   as_tibble()
genes<- genes %>%
  dplyr::select(c(gene, padj)) #selecting the columns gene and padj
  
all_genes_names <- genes$gene
...
#skip thisgenes<-t(genes)

#get a facttor with 2 levels--> named nummeric vector
...
##topGOtable
? what are the inputs I need to use? 
```{r}
...
bg_symbols <- mapIds(org.Hs.eg.db,
                     ...
                     keytype="ENSEMBL",
                     multiVals="first")
GOdata_2<-topGOtable(GOdata, bg_symbols,
                            ontology="BP", 
                            mapping = "org.Hs.eg.db",
                            geneID = "symbol") ##??? allgenes must be a factor with two levels
...
allRes <- GenTable(GOdata, classicFisher = resultFisher,
                    classicKS = resultKS, elimKS = resultKS.elim, #with elim method 
