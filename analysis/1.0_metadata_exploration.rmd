...
rld <-rld[rownames(rld) %in% rownames(results_PAS_ctrl)]
rld2 <- as_tibble(assay(rld[rownames(rld) %in% rownames(results_PAS_ctrl)])) |>
  mutate(id = rownames(rld)) |>
  column_to_rownames(var = "id")

clusters <- degPatterns(rld2, 
                        metadata = anno |> 
