...
<!-- VJM: I would suggest to save transformed/generated data from each notebook and load it again if needed in the next notebook (e.g. the transformed metadata, DEG results, cluster info, etc.). I find it difficult to follow what data from the previous steps you use for this analysis! Also this way you can start each notebook with a clean environment and don't have to worry about overwriting existing objects + makes analysis more robust/reproducible -->
...
library(pcaExplorer)
 ...
 #<!-- VJM: Just load the package don't install it -->
...
head(topDiffGenes(geneList))
 ...
 #<!-- VJM: I'm not sure I get this, this doesn't run with your data ?? -->
...
#<!-- VJM: If I recall correctly: ENS_sig is minus CtrlMus background genes and sig.res is unfiltered. Maybe it might be worth to think of better descriptive object names. Ideally, if you look at the analysis in a year the names should be obvious  -->
...
```{r}
#<!-- VJM: Is this what you wanted to do?  -->

# From the post you specified above (https://www.biostars.org/p/471549/):
# (1) rank genes by padj and and make vector with length of genes
rankedGenes <- c(1:nrow(ENS_sig))
names(rankedGenes) <- ENS_sig %>%
  arrange(padj) %>% pull(gene_name)
